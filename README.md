...
**Status: Day 7 — Blockchain & Cryptographic Procurement Ledger Integration.**
The platform features an end-to-end AI-powered verification pipeline coupled with an
immutable, tamper-evident cryptographic blockchain ledger, Proof-of-Existence (PoE)
document fingerprinting, Solidity EVM smart contract, and a zero-login Public Verifier portal.
...
- **Cryptographic Blockchain & Merkle Tree Ledger**:
  - SHA-256 block linking with Merkle tree transaction proofs.
  - Proof-of-Existence (PoE) fingerprinting for all uploaded bidder credentials and tender documents.
  - Interactive **Block Explorer** (`/blockchain`) with live chain integrity checking and inspection.
  - **Judge Demo Attack Simulator**: Demonstrates live tamper detection by injecting raw DB edits and showing how cryptographic seals catch unauthorized corruption.
  - **Zero-Login Public Verifier Portal** (`/verify`): Allows vigilance officers (CPCL / CVC), bidders, and third-party auditors to verify document hashes and compliance verdicts without authentication.
  - **Solidity Smart Contract** (`contracts/TenderComplianceLedger.sol`) ready for EVM deployment (Polygon / Ethereum Sepolia).
...
## 17. Upgraded V2 Features (SIH 2026 PS 26100 Full Implementation)

### 1. Dedicated Bidder/Vendor Portal
- **Dashboard (`/vendor/dashboard`)**: Live submission status, stage stepper (Submission → Evaluation → Clarifications → Verdict), and active bid tracking.
- **Document Management (`/vendor/documents`)**: Upload & re-upload documents with non-destructive versioning (`V1`, `V2`, `V3`), SHA-256 fingerprinting, tampering risk radar, and replacement uploads.
- **Clarification Notices (`/vendor/clarifications`)**: Direct communication channel for audit queries, allowing structured replies and replacement evidence submission.

### 2. Dedicated Procurement Officer Console
- **Officer Dashboard (`/officer/dashboard`)**: Unified control room showing live metrics (active tenders, compliant/non-compliant counts, pending clarifications, high-risk flags).
- **Multi-Bidder Compare & L1 Commercial Ranking (`/officer/compare`)**: Side-by-side technical compliance matrix separated from commercial price evaluation. Automated L1 formula calculation with statutory 18% GST + freight; strictly disqualifies technically non-compliant bidders from L1 eligibility.
- **Overrides & Deviations Ledger (`/officer/decisions`)**: Officer override workflow with mandatory justification and immutable anchoring to the cryptographic blockchain ledger.
- **Cartel & Tampering Risk Center (`/officer/risk`)**: Network graph analysis detecting synchronized submission timing (< 15 mins), price clustering (< 0.8%), and shared email domains, coupled with document quality & alteration alerts.
- **Clarification Committee (`/officer/clarifications`)**: Issue official inquiry notices to bidders with automated notification and resolution status.

### 3. Explainable AI Decision Breakdown ("Why this decision?")
- Instant evidence inspector showing:
  - Tender Mandated Requirement vs. Extracted Document Value
  - Deterministic Rule Applied (`evaluation_rule(...)`)
  - Human-readable Evaluation Reason
  - Extracted Text Evidence Snippet from uploaded records
  - AI Extraction Confidence Score (%)
  - Source Document & Page Reference
  - Blockchain Ledger Anchor Confirmation

### 4. Interactive AI Procurement Assistant
- Floating AI Assistant chatbot querying the live PostgreSQL database via `/api/assistant/chat`.
- Context-aware retrieval answering queries about L1 rankings, non-compliant bidders, mandatory failures, and blockchain proofs.
- Accessible across both public and authenticated views.

### 5. Demo Credentials
| Role | Email | Password | Portal View |
|---|---|---|---|
| Admin | `admin@cpcl.gem` | `Admin@123` | Full System Access, RBAC & Settings |
| Procurement Officer | `officer@cpcl.gem` | `Officer@123` | Officer Console, Compare & L1, Overrides, Risk |
| Bidder / Vendor | `vendor@cpcl.gem` | `Vendor@123` | Vendor Portal, Documents & Versions, Clarifications |
| Public Visitor | Zero Login Needed | N/A | Homepage (`/`), Public Verification (`/verify`) |
