...
async def upload_document(
    ...
    current_user: User = Depends(require_roles(UserRole.ADMIN, UserRole.EVALUATOR, UserRole.BIDDER)),
):
    ...
    # --- Cryptographic Blockchain Anchoring (Proof of Existence) ---
    try:
        ...
        if proof and hasattr(proof, "tx_hash"):
            v1.blockchain_tx = proof.tx_hash
            db.commit()
    except Exception as e:
        db.rollback()
        logger.warning(f"Blockchain anchoring warning for document {document.id}: {e}")
