...
class OEMAuthorizationVerificationProvider(VerificationProvider):
    """Mock OEM authorization / dealership-letter validity check."""

    name = "oem_authorization_mock"

    def verify(self, identifier, context):
        if not identifier:
            return ProviderResult(
                VerificationStatus.NOT_FOUND,
                note="No OEM authorization reference number could be read from the bidder's document.",
                provider_name=self.name,
            )
        key = identifier.strip().upper()
        record = mockdb.OEM_AUTHORIZATION_DATABASE.get(key)
        if not record:
            return ProviderResult(
                VerificationStatus.NOT_FOUND,
                note=f"OEM authorization {key} was not found in the mock OEM dataset (MOCK DATA).",
                provider_name=self.name,
            )
        ok = record["status"] == "Valid"
        return ProviderResult(
            VerificationStatus.VERIFIED if ok else VerificationStatus.MISMATCH,
            data={"oem_authorization_number": key, **record},
            note=(
                f"Mock OEM authorization registry shows {key} (OEM: {record.get('oem_name')}) "
                f"status = '{record['status']}', valid upto {record.get('valid_upto')}."
            ),
            provider_name=self.name,
        )
...
PROVIDER_REGISTRY: Dict[str, VerificationProvider] = {
    ...
    "oem_authorization": OEMAuthorizationVerificationProvider(),
...
IDENTIFIER_FIELD_BY_CATEGORY: Dict[str, str] = {
    ...
    "oem_authorization": "oem_authorization_number",
