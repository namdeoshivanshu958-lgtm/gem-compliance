      ...
      <div className="space-y-4">
        {!data ? (
          ...
          <>
            ...
            {data.type_mismatch && (
              <Alert variant="warning" title="Document classification discrepancy">
                Automated document analysis detected this file as a{' '}
                <strong>
                  {data.detected_document_type ? docLabel(data.detected_document_type) : 'different'}
                </strong>{' '}
                document type, which differs from the declared classification ({docLabel(document.document_type)}).
