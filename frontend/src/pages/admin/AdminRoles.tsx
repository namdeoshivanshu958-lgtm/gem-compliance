...
const ROLES: UserRole[] = ['admin', 'evaluator', 'viewer', 'bidder']
...
const ROLE_META: Record<UserRole, { icon: JSX.Element; tone: Tone }> = {
  ...
  bidder: { icon: <Building2 className="h-5 w-5" />, tone: 'warning' },
}
...
export default function AdminRoles() {
  ...
  useEffect(() => {
    ...
    listUsers()
      .then((users) => {
        const c: Record<UserRole, number> = { admin: 0, evaluator: 0, viewer: 0, bidder: 0 }
        users.forEach((u) => {
          c[u.role] += 1
        })
        setCounts(c)
